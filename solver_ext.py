    ...
    def solve(self):
        ...
        while fixed_cells_count != instance_size:
            ...
            if len(line_to_explore) == 0 and len(column_to_explore) == 0:
                stack = []
                undefined_cells = []
                undefined_cells_count = {}
                for i in range(instance_height):
                    for j in range(instance_width):
                        if self.instance[i][j] == self.Color.UNDEFINED:
                            undefined_cells.append((i, j))
                            undefined_cells_count[(i, j)] = 0
                            
                while len(undefined_cells) != 0:
                    i, j = undefined_cells[0]
                    
                    valid_line_coloring = False
                    valid_column_coloring = False
                    print(stack)
                    ...
                    if self.instance[i][j] != self.Color.WHITE:
                        ...
                        self.line_validity_trace = [[None] * len(self.cSeq) for i in range(len(self.lSeq[i]) + 1)]
                        self.column_validity_trace = [[None] * len(self.lSeq) for i in range(len(self.cSeq[j]) + 1)]
                        valid_line_coloring = self.compute_validity_trace(i, instance_width - 1, len(self.lSeq[i]), self.For.LINE)
                        valid_column_coloring = self.compute_validity_trace(instance_height - 1, j, len(self.cSeq[j]), self.For.COLUMN)
                        print("white", i,j,valid_line_coloring, valid_column_coloring)
                        print(self.line_validity_trace)

                    if not valid_line_coloring or not valid_column_coloring and self.instance[i][j] != self.Color.BLACK:
                        self.set_cell_color(i, j, self.Color.BLACK)
                        self.line_validity_trace = [[None] * len(self.cSeq) for i in range(len(self.lSeq[i]) + 1)]
                        self.column_validity_trace = [[None] * len(self.lSeq) for i in range(len(self.cSeq[j]) + 1)]
                        valid_line_coloring = self.compute_validity_trace(i, instance_width - 1, len(self.lSeq[i]), self.For.LINE)
                        valid_column_coloring = self.compute_validity_trace(instance_height - 1, j, len(self.cSeq[j]), self.For.COLUMN)
                        print("black", i,j,valid_line_coloring, valid_column_coloring)
                        print(self.line_validity_trace)
                        if not valid_line_coloring or not valid_column_coloring:
                            if len(stack) == 0:
                                return "ERROR"
                            else:
                                cell = stack.pop()
                                ...
                                undefined_cells.insert(0, cell)
                                self.set_cell_color(i, j, self.Color.UNDEFINED)
                        else:
                            undefined_cells_count[(i, j)] += 1
                            if undefined_cells_count[(i, j)] < 2:
                                stack.append((i, j))
                            undefined_cells.pop(0)
                    else:
                        undefined_cells_count[(i, j)] += 1
                        if undefined_cells_count[(i, j)] < 2:
                                stack.append((i, j))
...
presolver = DynamicSolver()
presolver.load_instance("instances/11.txt")
presolver.solve()
print(presolver.instance)
